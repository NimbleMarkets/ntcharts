...
func (m model) Init() tea.Cmd {
	...
	graph.DrawRows(&m.c1, m.cursor.Add(canvas.Point{1, -len(columnLens1)}), columnLens1, blockStyle)

	// draw data set 2
	m.c2.Clear()
	graph.DrawXYAxisLeft(&m.c2, m.cursor, axisStyle)
	graph.DrawRows(&m.c2, m.cursor.Add(canvas.Point{1, -len(columnLens2)}), columnLens2, blockStyle2)

	// draw data set 2 on top of data set 1
	m.c3.Clear()
	graph.DrawXYAxisLeft(&m.c3, m.cursor, axisStyle)
	graph.DrawRows(&m.c3, m.cursor.Add(canvas.Point{1, -len(columnLens1)}), columnLens1, blockStyle)
	graph.DrawRows(&m.c3, m.cursor.Add(canvas.Point{1, -len(columnLens2)}), columnLens2, blockStyle2)
	return nil
}
