...
func (m model) Init() tea.Cmd {
	columnLens1 := []float64{1 + 1.891, 2 + 2.694, 1.561 + 2.109, 5.889 + 6.836, 0, 6.708 + 8.943, 1.066 + 2.55, 8.242 + 3.513, 9.768 + 9.989, 4.372}
	columnLens2 := []float64{1, 8.561 + 4.848, 2.398 + 4.302, 8.822 + 1.149, 6.733 + 2.805, 0, 5 + 7.138, 10.385 + 2, 3.707 + 9.529, 4.272}

	// draw data set 1
	m.c1.Clear()
	graph.DrawXYAxisLeft(&m.c1, m.cursor, axisStyle) // demo X axis extending left
	graph.DrawRows(&m.c1, m.cursor.Add(canvas.Point{X: 1, Y: -len(columnLens1)}), columnLens1, blockStyle)

	// draw data set 2
	m.c2.Clear()
	graph.DrawXYAxisLeft(&m.c2, m.cursor, axisStyle)
	graph.DrawRows(&m.c2, m.cursor.Add(canvas.Point{X: 1, Y: -len(columnLens2)}), columnLens2, blockStyle2)

	// draw data set 2 on top of data set 1
	m.c3.Clear()
	graph.DrawXYAxisLeft(&m.c3, m.cursor, axisStyle)
	graph.DrawRows(&m.c3, m.cursor.Add(canvas.Point{X: 1, Y: -len(columnLens1)}), columnLens1, blockStyle)
	graph.DrawRows(&m.c3, m.cursor.Add(canvas.Point{X: 1, Y: -len(columnLens2)}), columnLens2, blockStyle2)
	return nil
}
...
func main() {
	...
	// canvas 1 shows rows set 1
	// canvas 2 draws rows set 2
	// canvas 3 draws rows set 2 on top of rows set 1
	// rows will replace existing ones if the total rune heights are the same
